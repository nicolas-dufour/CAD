...
from ..positional_embeddings import PositionalEmbedding, FourierEmbedding
...
class DiffusionUNet(nn.Module):
    def __init__(
        ...
        self.map_label = nn.Linear(label_dim, noise_channels) if label_dim > 0 else None
        ...
        for level, mult in enumerate(channel_mults):
            ...
            if level == 0:
                ...
                self.encoder[f"{res}x{res}_conv"] = nn.Conv2d(
                ...
                if encoder_type == "skip":
                    ...
                    self.encoder[f"{res}x{res}_aux_skip"] = nn.Conv2d(
                        c_aux, c_out, kernel_size=1
                    )
                if encoder_type == "residual":
                    self.encoder[f"{res}x{res}_aux_residual"] = nn.Sequential(
                        Resample(c_aux, down=True, resample_filter=resample_filter),
                        nn.Conv2d(c_aux, c_out, kernel_size=3, padding=1),
                    )
                    c_aux = c_out
        ...
        for level, mult in reversed(list(enumerate(channel_mults))):
            ...
            if decoder_type == "skip" or level == 0:
                ...
                self.decoder[f"{res}x{res}_aux_conv"] = nn.Conv2d(
                    c_out,
                    out_channels,
                    kernel_size=3,
                    padding=1,
...
class ResNetBlock(nn.Module):
    def __init__(
        ...
        # Time embedding
        self.timestep_embedder = nn.Linear(
        ...
        if out_channels != in_channels or resample_proj:
            skip.append(nn.Conv2d(in_channels, out_channels, kernel_size=1, padding=0))
        ...
        if self.num_heads:
            ...
            self.attn = nn.MultiheadAttention(out_channels, self.num_heads)
