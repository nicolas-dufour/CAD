...
class MixCondAndUncertainty(nn.Module):
    def __init__(
        self,
        label_dim,
        dim,
        ...
        self.confidence_mapper_type = confidence_mapper_type
        self.num_confidence_bins = num_confidence_bins
        self.map_label = nn.Linear(label_dim, dim * expansion)
        if confidence_mapper_type == "mlp":
            self.map_uncertainty = nn.Sequential(
                PositionalEmbedding(num_channels=dim, endpoint=True),
                nn.Linear(dim, dim * expansion),
                nn.SiLU(),
            )
        elif confidence_mapper_type == "embedding":
            self.map_uncertainty = nn.Sequential(
                nn.Embedding(num_confidence_bins, dim * expansion),
                nn.SiLU(),
            )
        else:
            raise ValueError(f"Invalid confidence_mapper_type {confidence_mapper_type}")
        self.map_uncertainty_scale = nn.Linear(dim * expansion, dim * expansion)
        self.map_uncertainty_shift = nn.Linear(dim * expansion, dim * expansion)
        # Initialize the weights of the layers to the identity matrix and the biases to zero
        torch.nn.init.zeros_(self.map_uncertainty_scale.weight)
        torch.nn.init.zeros_(self.map_uncertainty_scale.bias)
        torch.nn.init.zeros_(self.map_uncertainty_shift.weight)
        torch.nn.init.zeros_(self.map_uncertainty_shift.bias)

    def forward(self, labels, uncertainty):
        if self.confidence_mapper_type == "mlp":
            uncertainty_emb = self.map_uncertainty(uncertainty * self.scaling)
        else:
            uncertainty_emb = self.map_uncertainty(uncertainty)
        uncertainty_scale, uncertainty_shift = (
            self.map_uncertainty_scale(uncertainty_emb),
            self.map_uncertainty_shift(uncertainty_emb),
        )
        if self.scale_init == "identity":
            uncertainty_scale = uncertainty_scale + 1
        label_emb = self.map_label(labels)
        return uncertainty_scale * label_emb + uncertainty_shift
...
class CADRINClassCond(RINBackbone):
    def __init__(
        self,
        *args,
        mix_labels_and_confidence=True,
        mix_labels_and_confidence_scale_init=True,
        num_confidence_bins=10,
        confidence_mapper_type="mlp",
        **kwargs,
    ):
        self.mix_labels_and_confidence = mix_labels_and_confidence
        self.mix_labels_and_confidence_scale_init = mix_labels_and_confidence_scale_init
        self.confidence_mapper_type = confidence_mapper_type
        self.num_confidence_bins = num_confidence_bins
        super().__init__(*args, **kwargs)

    def init_cond_mapping(self, label_dim: int, latents_dim: int, num_cond_tokens: int):
        if self.mix_labels_and_confidence:
            self.map_confidence_level_and_labels = MixCondAndUncertainty(
                label_dim=label_dim,
                dim=latents_dim // 4,
                expansion=4,
                scale_init=self.mix_labels_and_confidence_scale_init,
                confidence_mapper_type=self.confidence_mapper_type,
            )

        else:
            if self.confidence_mapper_type == "embedding":
                self.map_confidence_level = nn.Embedding(
                    self.num_confidence_bins, latents_dim
                )
            elif self.confidence_mapper_type == "mlp":
                self.map_confidence_level = TimeEmbedder(
                    "positional", latents_dim // 4, expansion=4, time_scaling=1000.0
                )
            else:
                raise ValueError(
                    f"Invalid confidence_mapper_type {self.confidence_mapper_type}"
                )
            self.map_labels = nn.Linear(label_dim, latents_dim)
            self.map_confidence_level_and_labels = lambda labels, confidence_level: (
                self.map_labels(labels),
                self.map_confidence_level(confidence_level),
            )
            self.num_learned_latents -= 1

    def cond_mapping(self, batch) -> Tensor:
        cond = batch["label"]
        if "confidence" in batch:
            confidence_level = batch["confidence"].float()
        else:
            confidence_level = torch.ones(cond.shape[0], device=cond.device)
        if self.confidence_mapper_type == "embedding":
            confidence_level = (
                confidence_level * (self.num_confidence_bins - 1)
            ).long()
        out = self.map_confidence_level_and_labels(cond, confidence_level)
...
class CADRINTextCond(RINBackbone):
    ...
    def init_cond_mapping(self, label_dim: int, latents_dim: int, num_cond_tokens: int):
        ...
        self.confidence_embedder = TimeEmbedder(
            "positional", latents_dim // 4, time_scaling=1000.0, expansion=4
        )
        self.confidence_positional_embedding = nn.Parameter(
            torch.randn(latents_dim),
            requires_grad=True,
        )
        nn.init.trunc_normal_(
            self.confidence_positional_embedding, std=0.02, a=-2 * 0.02, b=2 * 0.02
        )
    ...
    def cond_mapping(self, batch) -> Tensor:
        ...
        embeddings = self.map_text_tokens(embeddings)
        confidence = self.confidence_embedder(
            batch["confidence"].to(embeddings.dtype)
        ).unsqueeze(1)
        if self.modulate_conditioning:
            scale_confidence, shift_confidence = torch.chunk(
                self.modulation(confidence).unsqueeze(1), 2, dim=-1
            )
            embeddings = embeddings * (1 + scale_confidence) + shift_confidence
        embeddings = torch.cat(
            [
                confidence
                + self.confidence_positional_embedding.unsqueeze(0)
                .unsqueeze(0)
        ...
        registers_mask = self.registers_confidence_mask.unsqueeze(0).expand(
            batch_size, -1
        )
        mask = torch.cat(
            [
