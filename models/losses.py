...
class DDPMLoss:
    def __init__(
        ...
        self.resample_by_confidence = resample_by_confidence
        self.sample_random_when_drop = sample_random_when_drop

    def __call__(self, preconditioning, network, batch):
        x_0 = batch["x_0"]
        batch_size = x_0.shape[0]
        device = x_0.device
        t = torch.rand(batch_size, device=device, dtype=x_0.dtype)
        ...
        if conditioning is not None and self.cond_drop_rate > 0:
            ...
            if "confidence" in batch:
                batch["confidence"] = (batch["confidence"]) * (
                    1 - drop_mask.to(batch["confidence"].dtype)
                )
        ...
        loss = (D_n - n) ** 2
        if self.resample_by_confidence:
            assert "confidence" in batch
            weight = batch["confidence"].to(loss.dtype).unsqueeze(-1).unsqueeze(
                -1
            ).unsqueeze(-1) / (batch["confidence"].to(loss.dtype).mean() + 1e-8)
            loss = loss * weight.to(loss.dtype)
        return loss

    def latent_self_cond(self, preconditioning, network, batch):
...
def randint_except(shape, low, high, except_array):
    samples = torch.randint(low, high - 1, size=shape, device=except_array.device)
